...
export function Skills() {
  ...
  return (
    <CommandBlock id="skills" command={t.skills.command}>
      <div className="space-y-8">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {skillCategories.map((category) => (
            <article
              key={category.id}
              className="rounded-md border border-border bg-surface/60 p-4 transition-colors hover:border-accent/35"
            >
              <p className="text-xs text-muted">$ {category.command}</p>
              <h2 className="mt-2 text-sm font-medium text-accent">
                {category.label[lang]}
              </h2>
